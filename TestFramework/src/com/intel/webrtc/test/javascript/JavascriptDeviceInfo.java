 ...
 *
 */
public class JavascriptDeviceInfo extends DeviceInfo{
    ...
    public JavascriptDeviceType getDeviceState() {
        return deviceState;
    }

    public void setDeviceState(JavascriptDeviceType deviceState) {
    ...
    public JavascriptDeviceInfo(String karmaConfigFile) throws IOException {
        ...
        if(brmat.find()){
            Logger.d(TAG, "Multiple browser options are founded in file:["+karmaConfigFile+"], javascript device init failed.");
            return;
        }
        ...
        if(kmmat.find()){
            Logger.d(TAG, "Multiple port options are founded in file:["+karmaConfigFile+"], javascript device init failed.");
            return;
        }
        ...
        if(tcmat.find()){
            ...
            Logger.d(TAG, "No test client source file is founded in file:["+karmaConfigFile+"], javascript device init failed.");
            return;
        }
        if(tcmat.find()){
            Logger.d(TAG, "Multiple test client source file are founded in file:["+karmaConfigFile+"], javascript device init failed.");
            return;
        }
...
//        System.out.println(deviceId+"\n"+browser+"\n"+karmaport+"\n"+testClient+"\n");
    ...
    public String toString() {
        String ret="JS test device, on broswer["+browser+"], state["+deviceState+"], "
                + "Logic Instance["+id+"], localPort["+localPort+"], testClient["+testClient+"].";
        return ret;
    }
}
